...
class Task {
  ...
  constructor(
    ...
    let task: JQuery = $("<div>", {
      ...
      taskTimestamp: JQuery = $("<div>", {
        html: `${Task.getDay(new Date(this.timestamp).getDay())} ${new Date(
          this.timestamp
        ).getDate()} 
               ${Task.getMonth(new Date(this.timestamp).getMonth())} ${new Date(
          this.timestamp
        ).getFullYear()}
               ${new Date(this.timestamp).getHours()}:${new Date(
          this.timestamp
        ).getMinutes()}`,
        class: "timestamp",
      }),
    ...
    taskContent.on({
      ...
      keydown: function (event): void {
        if (event.key === "Enter") $(this).trigger("focusout");
      },
    });
   ...
   */
  static getTaskIndex(task: HTMLElement | JQuery | null): number {
    let index = 0;
    for (let em of main_content.children(".task")) {
      if ($(em).text() === $(task as HTMLElement).text()) break;
      index++;
    }
    return index;
  }
